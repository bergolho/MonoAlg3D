...
SAVE_MESH(save_as_vtk_purkinje) {
    ...
    printf("\tOn 'save_as_vtk_purkinje'\n");
...
/*
    int num_points = sb_count(vtk_polydata->points);
    for (int i = 0; i < num_points; i++)
        printf("(%lf %lf %lf)\n",vtk_polydata->points[i].x,vtk_polydata->points[i].y,vtk_polydata->points[i].z);
    int num_lines = sb_count(vtk_polydata->lines);
    for (int i = 0; i < num_lines; i++)
        printf("%d -- %d\n",vtk_polydata->lines[i].source,vtk_polydata->lines[i].destination);
*/
    ...
    printf("\tLeaving 'save_as_vtk_purkinje'\n");
    exit(EXIT_FAILURE);
}